...
- Name: Negative Login Scenarios >> 3. GIVEN the login screen WHEN a user submits a missing user THEN the error should show Username is required
...
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝
Call log:
  - <launching> /ms-playwright/firefox-1482/firefox/firefox -no-remote -headless -profile /tmp/playwright_firefoxdev_profile-g1us8V -juggler-pipe -silent
  - <launched> pid=674
  - [pid=674][err] Running Nightly as root in a regular user's session is not supported.  ($HOME is /github/home which is owned by pwuser.)
  - [pid=674] <process did exit: exitCode=1, signal=null>
  - [pid=674] starting temporary directories cleanup
