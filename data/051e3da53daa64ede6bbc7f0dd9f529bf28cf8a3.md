...
- Name: Negative Login Scenarios >> 3. GIVEN the login screen WHEN a user submits incorrect user credentials THEN the error should show Username and password do not match any user in this service
- Location: /__w/spriteCloudAssessment/spriteCloudAssessment/tests/ui_tests.spec.ts:65:9
...
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝
Call log:
  - <launching> /ms-playwright/firefox-1482/firefox/firefox -no-remote -headless -profile /tmp/playwright_firefoxdev_profile-TfqLRF -juggler-pipe -silent
  - <launched> pid=713
  - [pid=713][err] Running Nightly as root in a regular user's session is not supported.  ($HOME is /github/home which is owned by pwuser.)
  - [pid=713] <process did exit: exitCode=1, signal=null>
  - [pid=713] starting temporary directories cleanup
